...
- **OpenAPI / Swagger spec file** (incl. large REST surfaces like Substrate API Sidecar, Aptos fullnode, Cosmos REST): **fetch the machine-readable spec itself and enumerate EVERY path/operation from it directly — do not transcribe from prose docs or sample a subset.** Pull the `paths` object (e.g. `curl -s <spec-url> | jq -r '.paths | keys[]'`, or the YAML equivalent) and emit one entry per path; for paths with multiple HTTP verbs, that path still counts once (the verb is captured separately). Then **verify your emitted count equals the spec's path count** — a shortfall means you sampled. A large REST API (50–150+ paths) is the single most common place under-counting happens; treat the full path list as a checklist, not a highlight reel.
- **Source-only or GitHub-only docs**: grep the source for handler / route registrations.
- **Multiple interfaces** (jsonrpc + REST + WebSocket subscriptions, etc.): enumerate per interface; do not flatten one interface's discoveries to omit another's. For dual-interface chains (e.g. Substrate jsonrpc + Sidecar REST) run the OpenAPI rule above on the REST side AND the per-family rule on the jsonrpc side; report a count for each interface separately.

**Capture alias and spelling variants the node actually serves.** When the docs (or the node's `rpc_methods`/`/spec` listing) expose more than one name for the same operation, emit BOTH — a spec missing an alias under-serves clients that call it. Common cases: British/American spelling pairs (`chain_getFinalisedHead` AND `chain_getFinalizedHead`), short aliases (`chain_getHead` for `chain_getBlockHash`, `account_nextIndex` for `system_accountNextIndex`), and documented legacy/deprecated aliases that are still routable. If the chain exposes a live `rpc_methods` (Substrate) or `/spec` (OpenAPI) endpoint, prefer enumerating from that authoritative machine list over prose docs, since it includes the aliases prose pages omit.
